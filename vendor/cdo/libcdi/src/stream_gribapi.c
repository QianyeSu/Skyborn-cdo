...
param_to_name(int param, char *name)
{
  ...
  if (pdis == 255) { snprintf(name, 256, "code%d", pnum); }
  else { snprintf(name, 256, "param%d.%d.%d", pnum, pcat, pdis); }
}
...
static void
gribapiInqProjRLL(grib_handle *gh, int gridID)
{
...
static void
gribapiInqProjLCC(grib_handle *gh, int gridID)
{
  ...
  if (isSouthPole && gribEditionNumber(gh) == 1)
  {
    gpp.lat_1 = -gpp.lat_1;
    gpp.lat_2 = -gpp.lat_2;
  }
...
static void
gribapiInqProjSTERE(grib_handle *gh, int gridID)
{
  struct CDI_GridProjParams gpp;
  gridProjParamsInit(&gpp);
...
static void
gribapiInqProjHEALPIX(grib_handle *gh, int gridID)
{
  struct CDI_GridProjParams gpp;
  gridProjParamsInit(&gpp);
...
static void
gribapiInqGridHEALPIX(grib_handle *gh, int gridID)
{
  long lval = -1;
  grib_get_long(gh, "Nside", &lval);
  int nside = (int) lval;
  ...
  int refinementLevel = log2(nside);
  cdiDefAttInt(gridID, CDI_GLOBAL, "refinement_level", CDI_DATATYPE_INT32, 1, &refinementLevel);
  const char *orderName = (order == 1) ? "nested" : "ring";
  cdiDefAttTxt(gridID, CDI_GLOBAL, "indexing_scheme", (int) (strlen(orderName)), orderName);
}
...
fdbScanTimesteps(stream_t *streamptr)
{
  ...
  for (int recID = 0; recID < numRecords; recID++)
  {
    ...
    if (CDI_Debug)
    {
      char paramstr[32];
      cdiParamToString(param, paramstr, sizeof(paramstr));
      Message("%4d name=%s id=%s ltype=%d lev1=%d lev2=%d vDateTime=%s", recID + 1, varname, paramstr, leveltype1, level1, level2,
...
gribapiScanTimestep2(stream_t *streamptr)
{
  ...
  for (int rindex = 0; rindex <= nrecords; ++rindex)
  {
    ...
    if (rindex == 0)
    {
      ...
      else { taxis->type = TAXIS_ABSOLUTE; }
      taxis->fDateTime = taxis->rDateTime;
      taxis->vDateTime = vDateTime;
      taxis->sDateTime = sDateTime;
    }
    ...
    if (CDI_Debug)
    {
      char paramstr[32];
      cdiParamToString(param, paramstr, sizeof(paramstr));
      Message("%4d %8lld name=%s id=%s ltype=%d lev1=%d lev2=%d vDateTime=%s", nrecsScanned, (long long) recpos, varname, paramstr,
    ...
    for (int rindex = 0; rindex <= nrecs; ++rindex)
    {
      ...
      if (CDI_Inventory_Mode != 1)
      {
        if (recinfo[recID].used)
        {
          ...
          if (CDI_Debug) gribWarning("Parameter already exist, skipped!", nrecsScanned, tsID + 1, varname, param, level1, level2);
      ...
      if (CDI_Debug) Message("%4d %8lld %4d %8d %8s", rindex + 1, (long long) recpos, param, level1, CdiDateTime_string(vDateTime));
              ...
              double missval)
{
  ...
  else { GRIB_CHECK(grib_get_double_array(gh, "values", (double *) data, &dummy), 0); }
...
gribapiDefStepUnits(int editionNumber, grib_handle *gh, int timeunit, int proDefTempNum, int gcinit)
{
  if (!gcinit)
  {
    ...
    if (editionNumber == 1) { grib_set_long(gh, "unitOfTimeRange", unitsOfTime); }
...
gribapiDefGridRegular(grib_handle *gh, int gridID, int gridtype, bool gridIsRotated, bool gridIsCurvilinear, int uvRelativeToGrid)
{
  ...
  else { GRIB_CHECK(my_grib_set_double(gh, "jDirectionIncrementInDegrees", yinc), 0); }
...
static void
gribapiDefProjLCC(grib_handle *gh, int editionNumber, int gridID, int uvRelativeToGrid)
{
...
static void
gribapiDefProjSTERE(grib_handle *gh, int editionNumber, int gridID, int uvRelativeToGrid)
{
  long xsize = (long) gridInqXsize(gridID);
  long ysize = (long) gridInqYsize(gridID);
...
static void
define_healpix_grid(grib_handle *gh, int nside, int order, int uvRelativeToGrid)
{
  static const char mesg[] = "healpix";
  size_t len = sizeof(mesg) - 1;
  GRIB_CHECK(my_grib_set_string(gh, "gridType", mesg, &len), 0);
  GRIB_CHECK(my_grib_set_long(gh, "Nside", nside), 0);
  GRIB_CHECK(my_grib_set_long(gh, "ordering", order), 0);
  GRIB_CHECK(my_grib_set_double(gh, "longitudeOfFirstGridPointInDegrees", 45.0), 0);
  if (uvRelativeToGrid >= 0) GRIB_CHECK(my_grib_set_long(gh, "uvRelativeToGrid", uvRelativeToGrid), 0);
}

static void
gribapiDefProjHEALPIX(grib_handle *gh, int gridID, int uvRelativeToGrid)
{
  struct CDI_GridProjParams gpp;
  gridInqParamsHEALPIX(gridID, &gpp);
  gridVerifyProjParamsHEALPIX(&gpp);

  define_healpix_grid(gh, gpp.nside, gpp.order, uvRelativeToGrid);
}
...
gribapiDefGridHEALPIX(grib_handle *gh, int gridID, int uvRelativeToGrid)
{
  ...
  int nside = 0;
  char attstring[256];
  if (cdiInqAttTxt(gridID, CDI_GLOBAL, "indexing_scheme", (int) sizeof(attstring), attstring) == 0)
  {
    int attlen = cdiInqAttLen(gridID, CDI_GLOBAL, "indexing_scheme");
    attstring[attlen] = 0;
    order = strStartsWith(attstring, "nest");
  }
  int attint = 0;
  if (cdiInqAttInt(gridID, CDI_GLOBAL, "refinement_level", 1, &attint) == 0) nside = lround(pow(2.0, attint));

  define_healpix_grid(gh, nside, order, uvRelativeToGrid);
}
...
gribapiDefPackingType(grib_handle *gh, bool lieee, bool lspectral, bool lcomplex, int comptype, SizeType gridsize)
{
  ...
  if (lspectral) { mesg = lcomplex ? mesg_spectral_complex : mesg_spectral_simple; }
  else if (comptype == CDI_COMPRESS_JPEG && gridsize > 1) { mesg = mesg_grid_jpeg; }
  else if ((comptype == CDI_COMPRESS_SZIP || comptype == CDI_COMPRESS_AEC) && gridsize > 1) { mesg = mesg_grid_ccsds; }
  else if (lieee) { mesg = mesg_ieee; }
...
gribapiDefGrid(int editionNumber, grib_handle *gh, int gridID, int comptype, int datatype, int uvRelativeToGrid)
{
  ...
  switch (gridtype)
  {
    ...
    case CDI_PROJ_LCC:
    {
      gribapiDefProjLCC(gh, editionNumber, gridID, uvRelativeToGrid);
      break;
    }
    case CDI_PROJ_STERE:
    {
      gribapiDefProjSTERE(gh, editionNumber, gridID, uvRelativeToGrid);
      break;
    }
    case CDI_PROJ_HEALPIX:
    {
      if (editionNumber <= 1) Error("HEALPix grid can't be stored in GRIB edition %d!", editionNumber);
      gribapiDefProjHEALPIX(gh, gridID, uvRelativeToGrid);
      break;
    }
    case GRID_HEALPIX:
    {
              ...
              double dlevel2)
{
  ...
  else { GRIB_CHECK(my_grib_set_long(gh, "level", lround(level)), 0); }
}
...
gribapiDefLevel(int editionNumber, grib_handle *gh, int zaxisID, int levelID, int gcinit, int proddef_template_num)
{
  ...
  switch (zaxistype)
  {
    ...
    case ZAXIS_HYBRID_HALF:
    {
      if (editionNumber <= 1) { grib_ltype = hasBounds ? GRIB1_LTYPE_HYBRID_LAYER : GRIB1_LTYPE_HYBRID; }
      defLevel(gh, gcinit, grib_ltype, grib_ltype, hasBounds, level, dlevel1, dlevel2);
    ...
    case ZAXIS_PRESSURE:
    {
      ...
      if (editionNumber <= 1)
      {
        ...
        {
          lwarn = false;
          Warning("GRIB1 can store only full pressure levels! Level changed from %ghPa to %dhPa.", level, (int) lround(level));
        }
      }
      else if (ltype2 == -1)
    ...
    case ZAXIS_SNOW:
      if (editionNumber <= 1)
        ;  // not available
      else { grib2DefLevel(gh, gcinit, grib_ltype, grib_ltype, hasBounds, level, dlevel1, dlevel2); }
    ...
    case ZAXIS_REFERENCE:
    {
      ...
      else
      {
        ...
        else { grib2DefLevel(gh, gcinit, grib_ltype, grib_ltype2, hasBounds, level, dlevel1, dlevel2); }
              ...
              int comptype, void *gribContainer)
{
  ...
  else { GRIB_CHECK(grib_set_double_array(gh, "values", (const double *) data, (size_t) datasize), 0); }
