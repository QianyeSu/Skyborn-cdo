...
print_vars_info(int operfunc, bool ensembleInfo, VarList const &varList, int vlistID, bool xsInfo)
{
  ...
  for (int varID = 0; varID < numVars; ++varID)
  {
    ...
    if (Options::cdoVerbose)
    {
      ...
      fprintf(stdout, " : %s", chunkSpecString.c_str());
    }
...
public:
  ...
  run() override
  {
    for (int fileIdx = 0; fileIdx < cdo_stream_cnt(); fileIdx++)
    {
      ...
      set_text_color(stdout, BRIGHT);
      fprintf(stdout, "%6d : Institut Source   T Steptype", -(fileIdx + 1));
      ...
      if (Options::cdoVerbose) fprintf(stdout, " : Chunkspec");
      reset_text_color(stdout);
      fprintf(stdout, "\n");
