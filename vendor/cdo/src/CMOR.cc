...
#endif
      cdo_abort(
          "ERROR (infile: '%s')! Program terminated by CMOR. A temporary ofile can outlive which needs to be deleted manually.",
...
cdo_parse_cmor_file(std::string const &filename, bool lismap)
{
  ...
  auto status = listBuffer.read(fp, filename.c_str());
  if (status && lismap)
...
file_exist(std::string const &tfilename, bool force, std::string const &fileart, bool print)
{
  ...
  FILE *fp = std::fopen(tfilename.c_str(), "r");
  if (fp == nullptr && force)
      ...
      if (attribute.empty())
        {
          if (!defaultstr.empty())
            ...
            cdo_abort(
                "ERROR (infile: '%s')! Function 'attribute check, compare and set':\n          Required value for attribute '%s' "
      ...
      if (cm[0] == 'n' && ((project_id == "CMIP5") || project_id == "CORDEX"))
      ...
      else
        {
          ...
          if (!lanyindex)
            {
              int scanres
                  = std::sscanf((const char *) vlabel.c_str(), "r%ldi%ldp%ld", &indexvalues[0], &indexvalues[1], &indexvalues[2]);
              if (!scanres)
                cdo_abort("ERROR (infile: '%s')!\n"
  ...
  else
    {
      ...
      if (!lanyindex)
      ...
      else if (!lallindices)
        {
          ...
          int scanres = std::sscanf((const char *) vlabel.c_str(), "r%ldi%ldp%ldf%ld", &vlvalues[0], &vlvalues[1], &vlvalues[2],
                                    &vlvalues[3]);
          if (!scanres)
      ...
      for (auto &kv : *kvl)
        {
          if (keep_this_attribute(&kv, kaa_notneeded_general) && keep_this_attribute(&kv, allneeded2)
              && keep_this_attribute(&kv, datasetvals) && keep_this_attribute(&kv, kaa_cmor2))
            {
              ...
              if (cmf != 0)
                cdo_abort("ERROR (infile: '%s')! Function cmor_set_cur_dataset_attribute failed!", cdo_get_stream_name(0));
            }
      ...
      if (nofvals)
        {
          if (!fa.empty())
            ...
            {
              cdo_abort(
                  "ERROR (infile: '%s')! No data to substitute found in infile for specified number of values '%d' for axis '%s'.",
                  ...
                  if (scanreturn == EOF || scanreturn == 0)
                    {
                      if (Options::cdoVerbose)
                        cdo_print(
                            "Could not convert the '%d'th value of the fourth spatial axis which is to merge into a double format.",
                            i);
                      break;
                    }
                }
          ...
          if (i == nofvals)
            {
              if (cmor_axis(new_axis_id(axis_ids), (char *) chardim.c_str(), (char *) funits.c_str(), nofvals, (void *) fvals, 'd',
                            fbnds, 2, NULL)
                  != 0)
                cdo_abort("ERROR (infile: '%s')! Could not register axis '%s' with double values.", cdo_get_stream_name(0),
                          chardim);
            }
  ...
  if (zsize > 1)
    {
      ...
      if (zaxisInqType(zaxisID) == ZAXIS_PRESSURE)
              ...
              if (project_id == "CMIP6")
                {
                  switch (miptab_freq)
                    ...
                    default:
                      ...
                      cmf = cmor_axis(new_axis_id(axis_ids), zaxisname2, (char *) zaxisunits, zsize, (void *) levels, 'd', nullptr,
                                      0, nullptr);
                      break;
                    }
                }
          ...
          if (zaxis == "hybrid_height")
            {
              ...
              cmf = cmor_axis(new_axis_id(axis_ids), (char *) "hybrid_height", (char *) "m", zsize, (void *) alev_val, 'd',
                              alev_bnds, 1, nullptr);
          ...
          else
            {
              ...
              cmf = cmor_axis(new_axis_id(axis_ids), (char *) "alternate_hybrid_sigma_half", (char *) "", zsize, (void *) alev_val,
                              'd', nullptr, 1, nullptr);
            }
          ...
          if (zaxistype == ZAXIS_HYBRID)
            {
              ...
              if (zaxis == "hybrid_height")
                {
                  cmf = cmor_zfactor(zfactor_id, lev_id, (char *) "orog", (char *) "m", count_axis_ids(axis_ids) - 2,
                                     hharray.data(), orogtype, (void *) orogdata, nullptr);
                }
              else if (time_method[0] == 'p')
                {
            ...
            {
              cmf = cmor_zfactor(zfactor_id, lev_id, (char *) "b_half", (char *) "", 1, &lev_id_array[0], 'd', (void *) b_val,
                                 nullptr);
        ...
        {
          if (zaxis == "alevel")
            cmf = cmor_axis(new_axis_id(axis_ids), (char *) "alt", (char *) zaxisunits, zsize, (void *) levels, 'd', zcell_bounds,
                            2, nullptr);
          ...
          if (cmor_time_name != "none")
            {
              ...
              if (*cmorTimeAxisID == CMOR_UNDEFID)
                {
                  cmf = cmor_axis(cmorTimeAxisID, (char *) cmor_time_name.c_str(), (char *) time_units.c_str(), 0, nullptr, 0,
                                  nullptr, 0, nullptr);
                  if (((project_id == "CMIP5") || (project_id == "CORDEX"))
          ...
          if (miptab_freq == 19)
            {
              ...
              register_z_axis(kvl, vlistID, varID, zaxisID, std::string(name), zaxis, axis_ids, &var->zfactor_id, project_id,
                              miptab_freq, mergeIDs, vars, streamID);
        ...
        if (ntsteps > 2)
          {
            ...
            else if (fp_is_equal(ntperyr, (double) 365) || fp_is_equal(ntperyr, (double) 365.25)
                     || fp_is_equal(ntperyr, (double) 366))
              ...
              for (size_t loopid = 1; loopid < ivsize; loopid++)
                {
                  ...
                  if (std::fabs(julianDate_to_seconds(julianDate_sub(jtime_val, julianDate_encode(calendar, vDateTimeCorr))))
                      < diff)
                    {
                      minid = loopid;
                      vDateTimeCorr.time = cdiTime_encode(iv[minid], 0, 0, 0);
                      diff
                          = std::fabs(julianDate_to_seconds(julianDate_sub(jtime_val, julianDate_encode(calendar, vDateTimeCorr))));
                    }
                }
                  ...
                  if (vars[i].zfactor_id > 0 && !l_hh)
                    {
                      ...
                      cmf = cmor_write(vars[i].zfactor_id, vars[psID].data, vars[psID].datatype, 1, &time_val, time_bndsp,
                                       &vars[i].cmor_varID);
#else
                      cmf = cmor_write(vars[i].zfactor_id, vars[psID].data, vars[psID].datatype, chunk_files[i], 1, &time_val,
      ...
      if (!vars[i].help_var)
        {
          ...
          if (std::strcmp(file_name, "") == 0)
            cdo_abort("ERROR (infile: '%s')! Function cmor_write failed!", cdo_get_stream_name(0));
#if (CMOR_VERSION_MAJOR == 2)
          if (project_id == "CORDEX")
            {
              if (kv_get_a_val(kvl, "tp", "y") == "y")
