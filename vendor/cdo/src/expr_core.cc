...
static double trimrel(double x, double keepbits) { // https://gmd.copernicus.org/articles/14/377/2021/
  ...
  int64_t  *b = (int64_t *) &x;
  int maskbits = 52 - (int) keepbits; // explicit bits in double precision
  int64_t mask = (0xFFFFFFFFFFFFFFFF>>maskbits)<<maskbits;
  int64_t half_quantum1 = ( 1<<(maskbits-1) ) - 1;
...
static double trimabs(double x, double max_abs_err) {//Sec. A6 https://gmd.copernicus.org/articles/14/377/2021/
  constexpr int64_t zerofrac = ~0ULL << 52; // zeros in place of mantissa in IEEE754 double
  constexpr double maxval = double(1ULL << 52); // max quantizable value in quanta
  //
  ...
  if (abs(x) < maxval*max_abs_err) { //quantisation is meaningful, no integer overflow
    int64_t *b = (int64_t *) (& max_abs_err); // access it as int64
    *b &= zerofrac; // zero fraction bits of xmax_abs_err: reduce it to benefit from compression
    double quantum = max_abs_err * 2; //quantum -- twice of that
...
func_var_var(int init, int funcID, nodeType *p1, nodeType *p2)
{
  ...
  if (!init)
  {
    ...
    for (size_t k = 0; k < nlev; ++k)
    {
      ...
      auto const *idat1 = p1->param.data + loff1;
      auto const *idat2 = p2->param.data + loff2;
      auto odat = p->param.data + loff;
      auto hasMV = (numMissVals1 > 0 || numMissVals2 > 0);
...
ex_ifelse(int init, nodeType *p1, nodeType *p2, nodeType *p3)
{
  ...
  if (!init)
  {
    ...
    for (size_t k = 0; k < nlev; ++k)
    {
      ...
      auto const *idat1 = pdata1 + loff1;
      auto const *idat2 = pdata2 + loff2;
      auto const *idat3 = pdata3 + loff3;
      auto odat = p->param.data + loff;
    ...
    for (size_t i = 0; i < param.ngp; ++i)
    {
      if (i < maxout || i >= (param.ngp - maxout))
      {
        ...
        if (param.steptype == TIME_CONSTANT)
          std::fprintf(stdout, "   %s[lev=%zu:gp=%zu] = %g\n", vname.c_str(), k + 1, i + 1, v);
        else
          std::fprintf(stdout, "   %s[ts=%ld:lev=%zu:gp=%zu] = %g\n", vname.c_str(), tsID, k + 1, i + 1, v);
      }
      else if (i == maxout) { std::fprintf(stdout, "   .......\n"); }
    }
}
