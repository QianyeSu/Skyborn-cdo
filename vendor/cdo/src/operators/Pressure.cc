...
public:
  ...
  run() override
  {
    ...
    Varray<double> deltaPressure(gridsize * numFullLevels);
    Varray<double> pressureFull(gridsize * numFullLevels);
    Varray<double> pressureHalf(gridsize * numHalfLevels);
    ...
    while (true)
    {
      ...
      if (zaxisID_ML != -1)
      {
        ...
        else if (varIDs.psID != -1) { varray_copy(gridsize, array, psProg); }
