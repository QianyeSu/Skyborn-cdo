...

{
  ...
  {
    std::fprintf(stderr, " No Velocity Components in input file, cannot creaate Vector PLOT!\n");
    return;
  }

  if (uarray.empty() || varray.empty())
  {
    std::fprintf(stderr, " Found only one Velocity Component in input file, cannot create Vector PLOT!\n");
    return;
  }

  if (DBG)
  {
    std::fprintf(stderr, "Num params %zu\n", params.size());
  ...
  for (i = 0; i < nparam; ++i)
  {
    ...
    if (key == "thin_fac")
    {
      ...
      if (DBG) std::fprintf(stderr, "THIN FACTOR %g\n", THIN_FAC);
    }

    if (key == "unit_vec")
    {
      UNIT_VEC = std::stof(value);
      if (DBG) std::fprintf(stderr, "UNIT VECTOR %g\n", UNIT_VEC);
    }
    ...
    if (key == "step_freq")
    {
      ...
      if (DBG) std::fprintf(stderr, "STEP FREQ %d\n", STEP_FREQ);
    }
  }
    ...
    while (true)
    {
      ...
      for (int fieldID = 0; fieldID < numFields; ++fieldID)
      {
        ...
        if (operatorID == VECTOR)
        {
          ...
          {
            if (DBG) std::fprintf(stderr, "Found U VEL in Varname %s\n", varname.c_str());
            size_t numMissVals;
            cdo_read_field(streamID, uarray.data(), &numMissVals);
          ...
          {
            if (DBG) std::fprintf(stderr, "Found V VEL in Varname %s\n", varname.c_str());
            size_t numMissVals;
            cdo_read_field(streamID, varray.data(), &numMissVals);
          ...
          std::fprintf(stderr, " Stream Operator Un-Supported!\n");
        else
          std::fprintf(stderr, " Operator Un-Supported!\n");
      }
      ...
      if (operatorID == VECTOR)
      {
        if (found == 2)
        {
          if (DBG) std::fprintf(stderr, "Found Both U & V VEL, Creating vector fields! \n");
        ...
        {
          std::fprintf(stderr, "No Velocity Components in input file, cannot create Vector PLOT!\n");
          break;
        }
      }
