...
struct CollgridInfo
{
  ...
  std::vector<GridInfo1> gridInfoList;
  VarList varList;
...
static int
gen_coll_grid(int numGrids, int numFiles, GridInfo2 const &gridInfo2, std::vector<CollgridInfo> &collgridInfoList, int gindex,
              long nxblocks)
{
  ...
  auto gridID = vlistGrid(collgridInfoList[0].varList.vlistID, gindex);
  ...
  {
    gridID = vlistGrid(collgridInfoList[fileIdx].varList.vlistID, gindex);
    auto gridType = (globalGridType != CDI_UNDEFID) ? globalGridType : gridInqType(gridID);
    if (gridType == GRID_LONLAT || gridType == GRID_GAUSSIAN || gridType == GRID_PROJECTION) { isRegular = true; }
    else if (gridType == GRID_CURVILINEAR) { isCurvilinear = true; }
    else if (gridType == GRID_UNSTRUCTURED) { isUnstructured = true; }
    else if (gridType == GRID_GENERIC /*&& gridInqXsize(gridID) > 0 && gridInqYsize(gridID) > 0*/) { isRegular = withCenter; }
  ...
  for (int fileIdx = 0; fileIdx < numFiles; ++fileIdx)
  {
    ...
    auto &collgridInfo = collgridInfoList[idx];

    if (isReg2D)
    {
      auto &gridInfo = collgridInfo.gridInfoList[gindex];
      gridInfo.nx = xsize[idx];
      gridInfo.ny = ysize[idx];
    ...
    for (long j = 0; j < ysize[idx]; ++j)
      for (long i = 0; i < xsize[idx]; ++i)
      {
        ...
        if (!isReg2D) collgridInfo.cellIndices[gindex][ij++] = offset + j * xsize2 + i;
      }
  }
  ...
  gridID = vlistGrid(collgridInfoList[0].varList.vlistID, gindex);
...
static void
collect_cells_reg2d(Field const &field1, Field &field2, GridInfo1 const &gridInfo, size_t nlon)
{
  auto nx = gridInfo.nx;
  auto ny = gridInfo.ny;
  ...
  {
    auto offset1 = j * nx;
    auto offset2 = gridInfo.offset + j * nlon;
...
{
  std::vector<GridInfo2> gridInfoList(varList.numGrids());
  ...
  if (globalCellIndicesID != -1) gridInfoList[varGridIndex[globalCellIndicesID]].globalIndicesID = globalCellIndicesID;
  if (globalVertIndicesID != -1) gridInfoList[varGridIndex[globalVertIndicesID]].globalIndicesID = globalVertIndicesID;
  if (globalEdgeIndicesID != -1) gridInfoList[varGridIndex[globalEdgeIndicesID]].globalIndicesID = globalEdgeIndicesID;

  for (auto const &var : varList.vars)
    if (selectedVars[var.ID]) gridInfoList[varGridIndex[var.ID]].needed = true;

  for (auto const &grid : gridInfoList)
  {
  ...
  for (int index = 0; index < varList.numGrids(); ++index)
  {
    ...
    gridInfoList[index].isReg2D = (gridType == GRID_LONLAT || gridType == GRID_GAUSSIAN || gridType == GRID_PROJECTION);
  }
...
public:
  ...
  std::vector<GridInfo2> gridInfoList;
  std::vector<CollgridInfo> collgridInfoList;
...
public:
  ...
  init() override
  {
    ...
    collgridInfoList.resize(numFiles);
    ...
    for (int fileIdx = 0; fileIdx < numFiles; ++fileIdx)
    {
      ...
      collgridInfoList[fileIdx].streamID = streamID;
      collgridInfoList[fileIdx].varList = VarList(vlistID);
    }

    auto const &varList1 = collgridInfoList[0].varList;
    vlistID1 = varList1.vlistID;
    vlistClearFlag(vlistID1);

    // check that the contents is always the same
    for (int fileIdx = 1; fileIdx < numFiles; ++fileIdx)
      varList_compare(varList1, collgridInfoList[fileIdx].varList, CmpVarList::Name | CmpVarList::NumLevels);
    ...
    gridInfoList = get_gridinfo(varList1, varGridIndex, selectedVars);
    ...
    for (int fileIdx = 0; fileIdx < numFiles; ++fileIdx)
    {
      auto &collGridInfo = collgridInfoList[fileIdx];
      collGridInfo.gridInfoList.resize(numGrids1);
    }
    ...
    {
      auto &collGridInfo = collgridInfoList[fileIdx];
      collGridInfo.cellIndices.resize(numGrids1);
      for (int gindex = 0; gindex < numGrids1; ++gindex)
      {
        if (gridInfoList[gindex].needed && gridInfoList[gindex].isReg2D == false)
        {
          auto patchSize = gridInqSize(vlistGrid(collGridInfo.varList.vlistID, gindex));
          collGridInfo.cellIndices[gindex].resize(patchSize);
        }
      }
    }
    ...
    for (int i2 = 0; i2 < numGrids2; ++i2)
    {
      ...
      gridID2s[i2] = gen_coll_grid(numGrids2, numFiles, gridInfoList[i1], collgridInfoList, i1, nxblocks);
      targetGridsize[i1] = gridInqSize(gridID2s[i2]);
      if (Options::cdoVerbose) cdo_print("Target grid%d size: %zu", i2 + 1, targetGridsize[i1]);
      if (gridInfoList[i1].isReg2D) { gridInfoList[i1].nx = gridInqXsize(gridID2s[i2]); }
    }
    ...
    collectVars2.resize(numVars2, false);
  ...
  run() override
  {
    ...
    int tsID = 0;
    do {
      numFields0 = cdo_stream_inq_timestep(collgridInfoList[0].streamID, tsID);
      for (int fileIdx = 1; fileIdx < numFiles; ++fileIdx)
      {
        auto numFields = cdo_stream_inq_timestep(collgridInfoList[fileIdx].streamID, tsID);
      ...
      for (int fieldID = 0; fieldID < numFields0; ++fieldID)
      {
        ...
        {
          std::tie(varID, levelID) = cdo_inq_field(collgridInfoList[fileIdx].streamID);
        }

        auto gindex = varGridIndex[varID];
        if (gridInfoList[gindex].needed && gridInfoList[gindex].isReg2D == false && gridInfoList[gindex].globalIndicesID == varID)
        {
...
#pragma omp parallel for default(shared)
          ...
          for (int fileIdx = 0; fileIdx < numFiles; ++fileIdx)
          {
            ...
            auto &collgridInfo = collgridInfoList[fileIdx];
            auto patchSize = collgridInfo.varList.vars[varID].gridsize;
            if (cellIndices.size() < patchSize) cellIndices.resize(patchSize);
            size_t numMissVals;
            cdo_read_field(collgridInfo.streamID, cellIndices.data(), &numMissVals);
            for (size_t i = 0; i < patchSize; ++i)
            {
              ...
              collgridInfo.cellIndices[gindex][i] = index - 1;
            }
          }
        }
...
#pragma omp parallel for default(shared)
          ...
          for (int fileIdx = 0; fileIdx < numFiles; ++fileIdx)
          {
            ...
            field1.init(collgridInfo.varList.vars[varID]);
            cdo_read_field(collgridInfo.streamID, field1);

            if (collectVars2[varID2])
            {
              if (gridInfoList[gindex].isReg2D)
                collect_cells_reg2d(field1, field2, collgridInfo.gridInfoList[gindex], gridInfoList[gindex].nx);
              else
                collect_cells(field1, field2, collgridInfo.cellIndices[gindex]);
            }
          }
  ...
  close() override
  {
    for (auto const &collgridInfo : collgridInfoList) cdo_stream_close(collgridInfo.streamID);
