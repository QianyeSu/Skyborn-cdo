...
public:
  ...
  inline static CdoModule module = {
    .name = "Eof3d",
    // clang-format off
    .operators = { { "eof3d", EOF3D_, 0, EofHelp },
                   { "eof3dspatial", EOF3D_SPATIAL, 0, EofHelp },
                   { "eof3dtime", EOF3D_TIME, 0, EofHelp } },
    // clang-format on
    .aliases = {},
...
public:
  ...
  init() override
  {
    ...
    datacounts.resize(numVars);
    datafields.resize(numVars);
    eigenvectors.resize(numVars);
    eigenvalues.resize(numVars);
    ...
    weights.resize(maxLevels * gridsizeMax, 1.0);
