...
static void
init_scale(Varray<double> &scale, int gridID, size_t gridsize, int operfunc)
{
...
public:
  ...
  inline static RegisterEntry<Arithlat> registration = RegisterEntry<Arithlat>();
    ...
    while (true)
    {
      ...
      for (int fieldID = 0; fieldID < numFields; ++fieldID)
      {
        ...
        if (gridID != gridID0)
        {
          ...
          init_scale(scale, gridID, gridsize, operfunc);
        }
