...
{
  std::fprintf(stdout, "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n");
  std::fprintf(stdout, "<kml xmlns=\"http://www.opengis.net/kml/2.2\" \n");
  std::fprintf(stdout, "     xmlns:atom=\"http://www.w3.org/2005/Atom\">\n");
  std::fprintf(stdout, "<Document>\n");
  std::fprintf(stdout, " <name>CDO plot</name>\n");
  std::fprintf(stdout, " <open>1</open>\n");
#ifdef VERSION
  std::fprintf(stdout, " <atom:generator>CDO %s</atom:generator>\n", VERSION);
#endif
  std::fprintf(stdout, " <description>\n");
  std::fprintf(stdout, " <![CDATA[Generated by CDO]]>\n");
  std::fprintf(stdout, " </description>\n");
  std::fprintf(stdout, " <LookAt>\n");
  std::fprintf(stdout, "	<longitude>0</longitude>\n");
  std::fprintf(stdout, "	<latitude>40</latitude>\n");
  std::fprintf(stdout, "	<range>6e+06</range>\n");
  std::fprintf(stdout, "	<tilt>0</tilt>\n");
  std::fprintf(stdout, "	<heading>0</heading>\n");
  std::fprintf(stdout, "	<altitudeMode>absolute</altitudeMode>\n");
  std::fprintf(stdout, " </LookAt>\n");
  std::fprintf(stdout, " <Style id=\"check-hide-children\">\n");
  std::fprintf(stdout, "  <ListStyle>\n");
  std::fprintf(stdout, "   <listItemType>checkHideChildren</listItemType>\n");
  std::fprintf(stdout, "  </ListStyle>\n");
  std::fprintf(stdout, " </Style>\n");
  std::fprintf(stdout, "<Folder>\n");
  std::fprintf(stdout, "<name>Layer:Page</name>\n");
  std::fprintf(stdout, "<open>0</open>\n");
  std::fprintf(stdout, " <styleUrl>#check-hide-children</styleUrl>\n");
  std::fprintf(stdout, "<TimeSpan>\n");
  std::fprintf(stdout, " <begin></begin>\n");
  std::fprintf(stdout, " <end></end>\n");
  std::fprintf(stdout, "</TimeSpan>\n");
  std::fprintf(stdout, "<description><![CDATA[Layer:Page]]></description>\n");
  std::fprintf(stdout, "<Folder>\n");
  std::fprintf(stdout, "<name>Layer:no_name</name>\n");
  std::fprintf(stdout, "<open>0</open>\n");
  std::fprintf(stdout, " <styleUrl>#check-hide-children</styleUrl>\n");
  std::fprintf(stdout, "<TimeStamp>\n");
  std::fprintf(stdout, " <when>2021-11-04T13:19:00Z</when>\n");
  std::fprintf(stdout, "</TimeStamp>\n");
  std::fprintf(stdout, "<styleUrl>#hiker-icon</styleUrl>\n");
  std::fprintf(stdout, "<description><![CDATA[Layer:no_name]]></description>\n");
  ...
  for (size_t i = 0; i < ngp; ++i)
  {
    ...
    std::fprintf(stdout, "<Placemark>\n");
    std::fprintf(stdout, "<visibility>1</visibility>\n");
    std::fprintf(stdout, "<open>0</open>\n");
    std::fprintf(stdout, "<Style>\n");
    std::fprintf(stdout, "<PolyStyle>\n");
    std::fprintf(stdout, "	<!-- r:%d g:%d b:%d -->\n", rgb[0], rgb[1], rgb[2]);
    std::fprintf(stdout, "	<color>fe%x%x%x</color>\n", (unsigned char) rgb[0], (unsigned char) rgb[1], (unsigned char) rgb[2]);
    std::fprintf(stdout, "	<fill>1</fill>\n");
    std::fprintf(stdout, "</PolyStyle>\n");
    std::fprintf(stdout, "<LineStyle>\n");
    std::fprintf(stdout, "	<width>2</width>\n");
    std::fprintf(stdout, "	<!-- r:%d g:%d b:%d -->\n", rgb[0], rgb[1], rgb[2]);
    std::fprintf(stdout, "	<color>fe%x%x%x</color>\n", (unsigned char) rgb[0], (unsigned char) rgb[1], (unsigned char) rgb[2]);
    std::fprintf(stdout, "</LineStyle>\n");
    std::fprintf(stdout, "</Style>\n");
    std::fprintf(stdout, "<MultiGeometry>\n");

    std::fprintf(stdout, "<Polygon>\n");
    std::fprintf(stdout, "<extrude>1</extrude>\n");
    std::fprintf(stdout, "<altitudeMode>clampToGround</altitudeMode>\n");
    std::fprintf(stdout, "<tessellate>0</tessellate>\n");
    std::fprintf(stdout, " <outerBoundaryIs>\n");
    std::fprintf(stdout, "  <LinearRing>\n");
    std::fprintf(stdout, "   <coordinates>\n");

    for (int ic = 0; ic < ncornerNew; ++ic) std::fprintf(stdout, "     %g,%g,%d\n", lonBounds[ic], latBounds[ic], height);
    std::fprintf(stdout, "     %g,%g,%d\n", lonBounds[0], latBounds[0], height);
  ...
  std::fprintf(stdout, "</Folder>\n");
  std::fprintf(stdout, "<ScreenOverlay id=\"legend\">\n");
  std::fprintf(stdout, "<name>Legend</name>\n");
  std::fprintf(stdout, "<Icon>\n");
  std::fprintf(stdout, " <href>legend.png</href>\n");
  std::fprintf(stdout, "</Icon>\n");
  std::fprintf(stdout, "<overlayXY x=\"0\" y=\"0\" xunits=\"fraction\" yunits=\"fraction\"/>\n");
  std::fprintf(stdout, "<screenXY x=\"0\" y=\"0\" xunits=\"fraction\" yunits=\"fraction\"/>\n");
  std::fprintf(stdout, "<size x=\"-1\" y=\"0.1\" xunits=\"fraction\" yunits=\"fraction\"/>\n");
  std::fprintf(stdout, "</ScreenOverlay>\n");
  std::fprintf(stdout, "</Folder>\n");
  std::fprintf(stdout, "</Document>\n");
  std::fprintf(stdout, "</kml>\n");
}
              ...
              Varray<double> const &vf)
{
  for (long j = 0; j < nlat; j += ninc)
    for (long i = 0; i < nlon; i += ninc)
    {
      ...
      if (std::fabs(auv) > 0) std::fprintf(stdout, " %g  %g  %g  %g\n", lon[idx], lat[idx], alpha, auv);
    }
...
public:
  ...
  init() override
  {
    ...
    */
    ...
    zaxis_center_lev.resize(nlev);
    zaxis_lower_lev.resize(nlev);
    zaxis_upper_lev.resize(nlev);
  ...
  run() override
  {
    ...
    while (true)
    {
...
#ifdef VERSION
        std::fprintf(stdout, "# Generated by CDO version %s\n", VERSION);
        std::fprintf(stdout, "#\n");
#endif
        std::fprintf(stdout, "# Operator = %s\n", cdo_operator_name(operatorID));
        // clang-format off
        if      (lhov) std::fprintf(stdout, "# Mode     = hovmoeller\n");
        else if (lzon) std::fprintf(stdout, "# Mode     = zonal\n");
        else if (lmer) std::fprintf(stdout, "# Mode     = meridional\n");
        else           std::fprintf(stdout, "# Mode     = horizonal\n");
        // clang-format on

        if (operatorID == OUTPUTVECTOR) std::fprintf(stdout, "# Increment = %d\n", ninc);
        std::fprintf(stdout, "#\n");
        std::fprintf(stdout, "# Stream = %s\n", cdo_get_stream_name(0));
        std::fprintf(stdout, "# Date   = %s\n", vdateString.c_str());
        std::fprintf(stdout, "# Time   = %s\n", vtimeString.c_str());
        std::fprintf(stdout, "# Name   = %s\n", var0.name.c_str());
        std::fprintf(stdout, "# Code   = %d\n", var0.code);
      }
      ...
      for (int fieldID = 0; fieldID < numFields; ++fieldID)
      {
        ...
        if ((tsID == 0 || lzon || lmer) && printHeader) std::fprintf(stdout, "# Level  = %g\n", level);
        if (lhov) std::fprintf(stdout, "# Timestep = %d\n", tsID + 1);

        if (printHeader) std::fprintf(stdout, "#\n");
        ...
        if (operatorID == GMTXYZ || operatorID == OUTPUTCENTER2 || operatorID == OUTPUTCENTERCPT)
        {
          if (Options::cdoVerbose)
          {
            ...
            std::fprintf(stderr, "makecpt -T%g/%g/%g -Crainbow > gmt.cpt\n", mm.min, mm.max, range / 20);
            std::fprintf(stderr, "pscontour -K -JQ0/10i -Rd -I -Cgmt.cpt data.gmt > gmtplot.ps\n");
            std::fprintf(stderr, "pscoast -O -J -R -Dc -W -B40g20 >> gmtplot.ps\n");
          }
          ...
          for (long i = 0; i < nvals; ++i)
          {
            ...
            if (operatorID == GMTXYZ)
            {
              if (lzon)
                ...
                std::fprintf(stdout, " %d  %g  %g\n", tsID + 1, lat, array[i]);
              else
                std::fprintf(stdout, " %g  %g  %g\n", lon, lat, array[i]);
            }
            else if (operatorID == OUTPUTCENTER2) { std::fprintf(stdout, " %g  %g  %g\n", plon[i], plat[i], parray[i]); }
            else
            {
              if (lzon)
                std::fprintf(stdout, " %g  %g  %g\n", lat, level, array[i]);
        ...
        else if (operatorID == GMTCELLS || operatorID == OUTPUTBOUNDSCPT)
        {
          if (Options::cdoVerbose)
          {
            ...
            auto range = mm.max - mm.min;
            std::fprintf(stderr, "makecpt -T%g/%g/%g -Crainbow > gmt.cpt\n", mm.min, mm.max, range / 20);
            std::fprintf(stderr, "psxy -K -JQ0/10i -Rd -L -Cgmt.cpt -m data.gmt > gmtplot.ps\n");
            // std::fprintf(stderr, "psxy -K -Jx0.028id -Rd -L -Cgmt.cpt -m
            // data.gmt > gmtplot.ps\n"); std::fprintf(stderr, "psxy -K
            // -JN0/10i -Rd -L -Cgmt.cpt -m data.gmt > gmtplot.ps\n");
            std::fprintf(stderr, "pscoast -O -J -R -Dc -W -B40g20 >> gmtplot.ps\n");
            std::fprintf(stderr, "ps2pdf gmtplot.ps\n");
          }
          ...
          for (size_t i = 0; i < gridsize; ++i)
          {
            ...
            if (!fp_is_equal(array[i], missval))
              std::fprintf(stdout, "> -Z%g", array[i]);
            else
              std::fprintf(stdout, "> -ZNaN");

            if (operatorID == OUTPUTBOUNDSCPT)
            {
              auto rgb = get_rgb(array[i], missval, cpt);
              std::fprintf(stdout, " -G%d/%d/%d", rgb[0], rgb[1], rgb[2]);
            }
            ...
            else
            {
              ...
              for (int ic = 0; ic < ncorner_new; ++ic) std::fprintf(stdout, "   %g  %g\n", lon_bounds[ic], lat_bounds[ic]);
