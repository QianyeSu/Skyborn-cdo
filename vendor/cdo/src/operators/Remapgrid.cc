...
static void
print_node_info(int gridID2, RemapVars const &rv, RemapGrid const &srcGrid)
{
  ...
  if (rv.numLinks == 1)
  {
    ...
    auto pointLL = srcGrid.get_lonlat(srcCellIndex);
...
public:
  ...
  bool remap_genweights = Options::RemapGenerateWeights;
  int numRemaps = 0;
  int numNeighbors = 0;
...
public:
  ...
  init() override
  {
    ...
    constexpr auto writeRemapWeightsOnly = false;
    remap_set_option(RemapOption::WriteWeights, writeRemapWeightsOnly);
    ...
    remap_set_option(RemapOption::GenerateWeights, (int) remap_genweights);
