...
flt2ibm(float x, unsigned char *ibm)
{
  ...
  if (exp < 0)
  {
    std::fprintf(stderr, "underflow in flt2ibm\n");
    ibm[0] = ibm[1] = ibm[2] = ibm[3] = 0;
    return 0;
  }
  if (exp > 127)
  {
    std::fprintf(stderr, "overflow in flt2ibm\n");
    ibm[0] = sign | 127;
...
dumpmap()
{
  ...
  {
    auto mapfobj = c_fopen(cdo_get_stream_name(0), "r");
    if (mapfobj.get() == nullptr) cdo_abort("Open failed on %s", cdo_get_stream_name(0));

    /* check the version number */

    fseek(mapfobj.get(), 1, 0);
    nbytes = std::fread(&vermap, sizeof(unsigned char), 1, mapfobj.get());
    ...
    if (vermap == 2)
    {
      fseek(mapfobj.get(), 2, 0);

      nbytes = std::fread(mrec, sizeof(unsigned char), 4, mapfobj.get());
      ...
      nbytes = std::fread(mrec, sizeof(unsigned char), 7, mapfobj.get());
      ...
      if (indx.hfnum > 0)
      {
        indx.hfpnt = new float[indx.hfnum];
        nbytes = std::fread(indx.hfpnt, sizeof(float), indx.hfnum, mapfobj.get());
      }
      if (indx.intnum > 0)
      {
      ...
      if (indx.fltnum > 0)
      {
        ...
        {
          nbytes = std::fread(urec, sizeof(unsigned char), 4, mapfobj.get());
    ...
    {
      fseek(mapfobj.get(), 0, 0);
      nbytes = std::fread(&indx, sizeof(struct gaindx), 1, mapfobj.get());
      ...
      {
        indx.hipnt = new int[indx.hinum];
        nbytes = std::fread(indx.hipnt, sizeof(int), indx.hinum, mapfobj.get());
      ...
      {
        indx.intpnt = new int[indx.intnum];
        nbytes = std::fread(indx.intpnt, sizeof(int), indx.intnum, mapfobj.get());
        if (swpflg) gabswp((float *) (indx.intpnt), indx.intnum);
      }
      if (indx.fltnum > 0)
      {
        indx.fltpnt = new float[indx.fltnum];
        nbytes = std::fread(indx.fltpnt, sizeof(float), indx.fltnum, mapfobj.get());
      ...
      if (indx.hipnt[0] == 4)
      {
        ...
        if (indxb.bignum > 0)
        {
          indxb.bigpnt = new off_t[indxb.bignum];
          nbytes = std::fread(indxb.bigpnt, sizeof(off_t), indxb.bignum, mapfobj.get());
...
static void
ctl_xydef(std::FILE *gdp, int gridID, bool *yrev)
{
  int j;
  ...
  *yrev = false;
  ...
  if (gridtype == GRID_PROJECTION && projtype == CDI_PROJ_LCC)
  {
    ...
    std::fprintf(gdp, "XDEF %d LINEAR %f %f\n", nx, xfirst, xinc);
    std::fprintf(gdp, "YDEF %d LINEAR %f %f\n", ny, yfirst, yinc);

    std::fprintf(gdp, "* XDEF 3600 LINEAR -179.95 0.1\n");
  ...
  else
  {
    ...
    if (is_equal(xinc, 0) && gridInqXvals(gridID, nullptr))
    {
      ...
      gridInqXvals(gridID, xvals.data());
      std::fprintf(gdp, "XDEF %d LEVELS ", xsize);
      j = 0;
      for (int i = 0; i < xsize; ++i)
      {
        std::fprintf(gdp, "%7.3f ", xvals[i]);
        j++;
        if (j == 6)
        {
          std::fprintf(gdp, "\n");
          j = 0;
          if (i != xsize - 1) std::fprintf(gdp, "               ");
        }
      }
      if (j) std::fprintf(gdp, "\n");
    }
    else
    {
  ...
  if (!(gridtype == GRID_PROJECTION && projtype == CDI_PROJ_LCC))
  {
    ...
    if (is_equal(yinc, 0) && gridInqYvals(gridID, nullptr))
    {
      ...
      gridInqYvals(gridID, yvals.data());
      std::fprintf(gdp, "YDEF %d LEVELS ", ysize);
      j = 0;
      if (yvals[0] > yvals[ysize - 1])
      {
        *yrev = true;
        ...
        {
          std::fprintf(gdp, "%7.3f ", yvals[i]);
          j++;
          if (j == 6)
          {
            std::fprintf(gdp, "\n");
            j = 0;
            if (i != 0) std::fprintf(gdp, "               ");
          }
        }
      }
      else
...
static void
ctl_zdef(std::FILE *gdp, int vlistID, bool *zrev)
{
  int j;
  int zaxisIDmax = -1;
  int zaxisID, nlev;
  ...
  *zrev = false;
  ...
  if (is_not_equal(levinc, 0))
    std::fprintf(gdp, "ZDEF %d LINEAR %g %g\n", nlevmax, level0, levinc);
  else
  {
    std::fprintf(gdp, "ZDEF %d LEVELS ", nlevmax);
    j = 0;
    /* zrev not needed !!!
    if ( *zrev )
      {
        for (i = nlevmax-1; i >= 0; i--)
          {
            if ( lplev ) std::fprintf(gdp, "%g ", levels[i]/100);
            else         std::fprintf(gdp, "%d ", (int) levels[i]);
            j++;
            if ( j == 10 )
              {
                std::fprintf(gdp, "\n");
    ...
    */
    {
      ...
      {
        if (lplev)
          std::fprintf(gdp, "%g ", levels[i] / 100);
        else
          std::fprintf(gdp, "%g ", levels[i]);
        j++;
        if (j == 10)
        {
          std::fprintf(gdp, "\n");
          j = 0;
          if (i != (nlevmax - 1)) std::fprintf(gdp, "               ");
        }
      }
    }
    if (j) std::fprintf(gdp, "\n");
...
static void
ctl_options(std::FILE *gdp, bool yrev, bool zrev, bool sequential, bool bigendian, bool littleendian, bool flt64, bool cal365day)
{
  /* if ( filetype == CDI_FILETYPE_GRB ) zrev = false; */

  if (yrev || zrev || sequential || bigendian || littleendian || flt64)
  {
    std::fprintf(gdp, "OPTIONS");
    if (yrev) std::fprintf(gdp, " yrev");
    if (zrev) std::fprintf(gdp, " zrev");
    if (sequential) std::fprintf(gdp, " sequential");
    if (bigendian) std::fprintf(gdp, " big_endian");
    if (littleendian) std::fprintf(gdp, " little_endian");
    if (flt64) std::fprintf(gdp, " flt64");
    if (cal365day) std::fprintf(gdp, " 365_day_calendar");
    std::fprintf(gdp, "\n");
  }
}

static void
ctl_undef(std::FILE *gdp, double missval)
{
  std::fprintf(gdp, "UNDEF  %g\n", missval);
}

static void
ctl_vars(std::FILE *gdp, int filetype, int vlistID, VarList const &varList, int nvarsout, int *vars)
{
  char varname[CDI_MAX_NAME];

  std::fprintf(gdp, "VARS  %d\n", nvarsout);
  ...
  for (int varID = 0; varID < numVars; ++varID)
  {
    if (vars[varID] == true)
    {
      ...
      std::strcpy(varname, var.name.c_str());
      int len = (int) std::strlen(varname);
      int i;
      for (i = 0; i < len; ++i)
        if (varname[i] == '-') break;
      ...
      std::fprintf(gdp, "%-15s", varname);

      if (nlev == 1) nlev = 0;

      std::fprintf(gdp, "  %3d", nlev);
      ...
      if (filetype == CDI_FILETYPE_GRB)
      {
        ...
        */
        std::fprintf(gdp, "  %d,%d", var.code, ltype);
      }
      else if (filetype == CDI_FILETYPE_NC)
      {
        ...
        std::fprintf(gdp, "  ");
        if (!var.isConstant) std::fprintf(gdp, "t,");
        if (xyz == 321)
        {
          if (nlev > 0) std::fprintf(gdp, "z,");
          std::fprintf(gdp, "y,x");
        }
        else if (xyz == 312)
        {
      ...
      if (var.longname.size())
        std::fprintf(gdp, "  %s", var.longname.c_str());
      else
        std::fprintf(gdp, "  %s", varname);

      if (var.units.size()) std::fprintf(gdp, "  [%s]", var.units.c_str());
...
write_map_grib1(const char *ctlfile, int map_version, int nrecords, int *intnum, float *fltnum, off_t *bignum)
{
  ...
  auto mapfobj = c_fopen(ctlfile, "w");
  if (mapfobj.get() == nullptr) cdo_abort("Open failed on %s", ctlfile);
  ...
  if (map_version == 2)
  {
    ...
    unsigned char *map = new unsigned char[nb];
    ...
    std::fwrite(map, 1, bcnt, mapfobj.get());

    delete[] map;
  }
  else
  {
    std::fwrite(&indx, sizeof(struct gaindx), 1, mapfobj.get());
    if (indx.hinum > 0) std::fwrite(hinum, sizeof(int), indx.hinum, mapfobj.get());
    if (map_version == 1)
    {
      ...
      if (indx.intnum > 0) std::fwrite(intnumbuf.data(), sizeof(int), indx.intnum, mapfobj.get());
      if (indx.fltnum > 0) std::fwrite(fltnum, sizeof(float), indx.fltnum, mapfobj.get());
    }
    else
    {
      if (indx.intnum > 0) std::fwrite(intnum, sizeof(int), indx.intnum, mapfobj.get());
      if (indx.fltnum > 0) std::fwrite(fltnum, sizeof(float), indx.fltnum, mapfobj.get());
      if (indxb.bignum > 0) std::fwrite(bignum, sizeof(off_t), indxb.bignum, mapfobj.get());
    }
  }
}
...
public:
  ...
  std::vector<std::string> IncrKey = { "mn", "hr", "dy", "mo", "yr" };
  ...
  Varray<double> array;
  std::vector<std::string> cmons = { "jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec" };
  ...
  int filetype{};
  int nvarsout = 0, numFieldsout = 0;

  VarList varList;

  std::vector<int> vars;
  std::vector<int> recoffset;
      ...
      if (filetype == CDI_FILETYPE_GRB)
      {
        ...
        for (int fieldID = 0; fieldID < numFields; ++fieldID)
        {
          ...
          if (vars[varID] == true)
          {
            ...
            index = (tsID * numFieldsout + recoffset[varID] + levelID);

            cdo_inq_grib_info(streamID, &intnum[index], &fltnum[index * 3], &bignum[index * 2]);

            if (map_version != 4)
            {
              long checksize = (long) bignum[index * 2] + (long) varList.gridsizeMax() * intnum[index] / 8;
...
public:
  ...
  init() override
  {
    ...
    auto len = std::strlen(datfile);
    ctlfile = new char[len + 10];
    std::strcpy(ctlfile, datfile);
    ...
    {
      if (FileUtils::size(cdo_get_stream_name(0)) > 2147483647UL) map_version = 4;
    }
    ...
    if (index == numGrids) cdo_abort("No Lon/Lat, Gaussian or Lambert grid found (%s data unsupported)!", gridNamePtr(gridtype));

    // select all variables with used gridID
    vars.resize(numVars);
    recoffset.resize(numVars);
    for (int varID = 0; varID < numVars; ++varID)
    {
      auto const &var = varList.vars[varID];
      if (var.gridID == gridID)
      {
        ...
        vars[varID] = true;
        recoffset[varID] = numFieldsout;
        nvarsout++;
        numFieldsout += zaxisInqSize(var.zaxisID);
        if (numSteps != 1 && numSteps != 0 && var.isConstant)
    ...
    if (filetype != CDI_FILETYPE_GRB && numVars != nvarsout) cdo_abort("Too many different grids!");
    ...
    // open ctl file
    gdp = std::fopen(ctlfile, "w");
    if (gdp == nullptr) cdo_abort("Open failed on %s", ctlfile);

    // VERSION
    std::fprintf(gdp, "* Generated by CDO operator gradsdes\n");
    std::fprintf(gdp, "*\n");

    // DSET
    if (datfile[0] == '/')
      std::fprintf(gdp, "DSET  %s\n", datfile);
    else
    {
    ...
    if (filetype == CDI_FILETYPE_GRB || filetype == CDI_FILETYPE_GRB2)
    {
      ...
      array.resize(varList.gridsizeMax());
    }
    else if (filetype == CDI_FILETYPE_NC) { std::fprintf(gdp, "DTYPE  NetCDF\n"); }

    // XYHEADER
    if (xyheader) std::fprintf(gdp, "XYHEADER  %d\n", xyheader);

    // TIME

    taxisID = vlistInqTaxis(vlistID);

    cal365day = (taxisInqCalendar(taxisID) == CALENDAR_365DAYS);
  }
  ...
  run() override
  {
    ...
    std::snprintf(Time, sizeof(Time), "%02d:%02dZ%02d%s%04d", ihh0, imn0, idd0, cmons[imm0 - 1].c_str(), iyy0);
    std::snprintf(Incr, sizeof(Incr), "%d%s", dt, IncrKey[iik].c_str());

    std::fprintf(gdp, "TDEF %d LINEAR %s %s\n", tsID, Time, Incr);
    ...
    if (res)
      std::fprintf(gdp, "TITLE  %s  T%d grid\n", datfile, res);
    else
      std::fprintf(gdp, "TITLE  %s  %dx%d grid\n", datfile, xsize, ysize);
