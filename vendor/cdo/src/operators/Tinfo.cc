...
print_bounds(int taxisID, int calendar)
{
  ...
  std::fprintf(stdout, " %s %s", date_to_string(vDateTime0.date).c_str(), time_to_string(vDateTime0.time).c_str());
  std::fprintf(stdout, " %s %s", date_to_string(vDateTime1.date).c_str(), time_to_string(vDateTime1.time).c_str());
  ...
  // std::fprintf(stdout, "  %g  %g  %g  %d", jdelta, jdelta/3600, std::fmod(jdelta,3600), timeIncr.period%3600);
  int len = std::fprintf(stdout, " %3ld %s%s", (long) timeIncr.period, time_units_cstr(timeIncr.units),
                    (std::abs(timeIncr.period) != 1) ? "s" : "");
  for (int i = 0; i < 11 - len; ++i) std::fprintf(stdout, " ");
}
...
public:
  ...
  init() override
  {
    ...
    if (numSteps != 0)
    {
      if (numSteps == CDI_UNDEFID)
        std::fprintf(stdout, "   Time axis :  unlimited steps\n");
      else
        std::fprintf(stdout, "   Time axis :  %d step%s\n", numSteps, (numSteps == 1) ? "" : "s");
      ...
      if (taxisID != CDI_UNDEFID)
      {
        if (taxisInqType(taxisID) != TAXIS_ABSOLUTE)
        {
          ...
          if (unit != CDI_UNDEFID) std::fprintf(stdout, "  Units = %s", tunit_to_cstr(unit));
          ...
          if (taxisInqType(taxisID) == TAXIS_FORECAST)
          {
            ...
            std::fprintf(stdout, "     Forecast RefTime = %s", datetime_to_string(fDateTime).c_str());
      ...
      std::fprintf(stdout, "\n");
      std::fprintf(stdout, "         Verification Time              ");
      if (lforecast) std::fprintf(stdout, " Forecast Reference Time     ");
      if (taxisHasBounds(taxisID)) std::fprintf(stdout, " lower bound          upper bound");
      std::fprintf(stdout, "\n");

      std::fprintf(stdout, "Timestep YYYY-MM-DD hh:mm:ss   Increment");
      if (lforecast) std::fprintf(stdout, " YYYY-MM-DD hh:mm:ss   Period");
      if (taxisHasBounds(taxisID)) std::fprintf(stdout, " YYYY-MM-DD hh:mm:ss  YYYY-MM-DD hh:mm:ss  Difference");
      std::fprintf(stdout, "\n");
    }
  }
  ...
  run() override
  {
    ...
    while (true)
    {
      ...
      std::fprintf(stdout, "%6d  %s %s", tsID + 1, date_to_string(vDateTime.date).c_str(), time_to_string(vDateTime.time).c_str());
      ...
      {
        auto fDateTime = taxisInqFdatetime(taxisID);
        std::fprintf(stdout, " %s", datetime_to_string(fDateTime).c_str());

        auto fc_period = taxisInqForecastPeriod(taxisID);
        std::fprintf(stdout, " %7g", fc_period);
      }
      ...
      if (tsID > 1 && timeIncr != timeIncr0)
      {
        ...
        if (its > 0)
        {
          ...
          if (Options::cdoVerbose)
            std::fprintf(stdout, "  %c--- Gap %d, missing %s%d timestep%s", arrow, numGabs, (its >= LimNTSM) ? "more than " : "", its,
                    (its != 1) ? "s" : "");
        }
        else if (its < 0)
        {
          if (Options::cdoVerbose) std::fprintf(stdout, "  %c--- Wrong date/time information, negative increment!", arrow);
        }
      }
    ...
    if (Options::cdoVerbose && numGabs)
    {
      ...
      {
        numGabs = MaxGaps;
        std::fprintf(stdout, ", here are the first %d", numGabs);
      }
      std::fprintf(stdout, ":\n");
      for (int igap = 0; igap < numGabs; ++igap)
      {
        std::fprintf(stdout, "  Gap %d between timestep %d and %d, missing %d timestep%s", igap + 1, rangetsm[igap][0],
                rangetsm[igap][1], ntsm[igap], (ntsm[igap] != 1) ? "s" : "");
        if (ntsm[igap] >= MaxNTSM)
        {
          ntsm[igap] = MaxNTSM;
          std::fprintf(stdout, ", here are the first %d", ntsm[igap]);
        }
        std::fprintf(stdout, ":\n");
