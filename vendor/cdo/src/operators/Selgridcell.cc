...
gen_grid_healpix_index(int gridID1, size_t gridsize2, std::vector<int64_t> const &cellIndices)
{
  ...
  if (gridsize1 == gridInqIndices(gridID1, nullptr))
  {
    std::vector<int64_t> indices(gridsize1);
    gridInqIndices(gridID1, indices.data());
    for (size_t i = 0; i < gridsize2; ++i) { indices[i] = indices[cellIndices[i]]; }
    gridDefIndices(gridID2, indices.data());
  }
  else { gridDefIndices(gridID2, cellIndices.data()); }
...
static int
gen_grid_generic(int gridID1, size_t gridsize2, std::vector<int64_t> const &cellIndices)
{
  (void) gridID1;
  (void) cellIndices;
  auto gridID2 = gridCreate(GRID_UNSTRUCTURED, gridsize2);
...
genindexgrid(int gridID1, size_t gridsize2, std::vector<int64_t> const &cellIndices)
{
  ...
  if (gridtype1 == GRID_UNSTRUCTURED || gridtype1 == GRID_CURVILINEAR)
    ...
    gridID2 = gengridcell(gridID1, gridsize2, cellIndices);
  else if (gridtype1 == GRID_GENERIC && !gridHasCoordinates(gridID1))
    gridID2 = gen_grid_generic(gridID1, gridsize2, cellIndices);
...
public:
  ...
  init() override
  {
    ...
    processVars.resize(numVars, false);

    auto numGrids = vlistNumGrids(vlistID1);
    sindex.resize(numGrids);
    ...
    for (int index = 0; index < numGrids; ++index)
    {
      ...
      auto gridID2 = genindexgrid(gridID1, numCells, cellIndices);
      if (gridID2 == -1)
      {
