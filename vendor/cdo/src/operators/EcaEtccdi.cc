                     ...
                     std::vector<FieldInfo> const &fieldInfoList, int selection, int func2)
{
  ...
  while (true)
  {
    ...
    if (year != recentYear)
    {
      writeTimesteps(MaxMonths, recentYear, cei, frequency, taxisID4, streamID4, otsID, varList1, fieldInfoList, tempdpm, tempdpy,
                     func2);
      recentYear = year;
      tempdpy = 0;
      tempdpm[0] = 0;
...
etccdi_op(ETCCDI_REQUEST &request)
{
  ...
  while (true)
  {
    ...
    if (numFields != cdo_stream_inq_timestep(streamID3, tsID))
      cdo_abort("Number of fields at time step %d of %s and %s differ!", tsID + 1, cdo_get_stream_name(1), cdo_get_stream_name(2));
  ...
  while (true)
  {
    ...
    if (year >= request.startboot && year <= request.endboot)
    {
      ...
      if (wdaysSrc[dayOfYear] || request.func2 == FieldFunc_Sum)
      {
        ...
        wdaysRead[dayOfYear + (year - request.startboot) * (MaxDays - 1)] = dayOfYear + (year - request.startboot) * (MaxDays - 1);
              ...
              for (int loopdoy = 1; loopdoy < MaxDays; loopdoy++)
              {
                ...
                if (wdaysRead[loopdoy + (bootsyear - request.startboot) * (MaxDays - 1)])
                {
                  ...
                  if (request.func2 == FieldFunc_Avg)
                  {
                    ...
                    for (size_t i = 0; i < toCompare.size; ++i) array[i] = (fp_is_equal(array[i], toCompare.missval)) ? 0.0 : 1.0;
                  }
                  else
                  {
                    auto &array = toCompare.vec_d;
...
#pragma omp critical
#endif
                    field2_add(cei[(bootsyear - request.startboot) * MaxMonths + (int) ((loopdoy - 1) / 31.)][varID][levelID],
                               toCompare);
                  else
#ifdef _OPENMP
#pragma omp critical
...
public:
  ...
  inline static CdoModule module = {
    .name = "EcaEtccdi",
    .operators = { { "etccdi_tx90p", func_selge, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi_tx10p", func_selle, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi_tn90p", func_selge, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi_tn10p", func_selle, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi_r95p", func_selge, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi_r99p", func_selge, CMP_DATE, Eca_etccdiHelp },
                   { "etccdi", 0, CMP_DATE, Eca_etccdiHelp } },
    .aliases = {},
