...
public:
  ...
  inline static auto registration = RegisterEntry<Eofcoeff>();
  ...
  run() override
  {
    ...
    while (1)
    {
      ...
      /*for ( eofID=0; eofID<neof; eofID++)
        {
          std::fprintf(stderr, "defining ts %i\n", tsID);
      ...
      */
      for (int fieldID = 0; fieldID < numFields; ++fieldID)
      {
        ...
        for (int eofID = 0; eofID < neof; eofID++)
        {
          ...
          // if ( fieldID == 0 ) std::fprintf(stderr, "ts%i rec%i eof%i\n", tsID, fieldID, eofID);
          out.vec_d[0] = 0;
          out.grid = gridID3;
