...
lprintf(std::FILE *fp)
{
  ...
  std::fprintf(fp, " ");
  for (int inum = 0; inum < num; inum++) std::fprintf(fp, "%c", cval);
  std::fprintf(fp, "\n");
}
  ...
  if (globs.EndOfInterval)
  {
    if (lstdout)
    {
      if (globs.OutputInterval == DAILY_INTERVAL)
        std::fprintf(stdout, " Processed Day %2d  Month %2d  Year %04d", globs.OldDate.dy, globs.OldDate.mo, globs.OldDate.yr);
      else if (globs.OutputInterval == MONTHLY_INTERVAL)
        std::fprintf(stdout, " Processed Month %2d  Year %04d", globs.OldDate.mo, globs.OldDate.yr);
      else if (globs.OutputInterval == UNLIM_INTERVAL)
        std::fprintf(stdout, " Processed range from %6.4d-%2.2d-%2.2d to %6.4d-%2.2d-%2.2d", globs.StartDate.yr, globs.StartDate.mo,
                     globs.StartDate.dy, globs.OldDate.yr, globs.OldDate.mo, globs.OldDate.dy);

      if (globs.Mean)
        std::fprintf(stdout, "  (Mean of %3d Terms)\n", globs.MeanCount);
      else
        std::fprintf(stdout, "   Terms %3d\n", globs.MeanCount);
    }
...
after_SwitchFile(AfterControl *globs)
{
  ...
  if (globs->Multi > 0)
  {
    int i = std::strlen(ifile);
    if (i < 10)
    {
      std::fprintf(stderr, " Not a valid filename: %s \n", ifile);
      std::exit(1);
    }
  ...
  std::fprintf(stderr, " Continuation file: %s\n", ifile);
...
after_printProcessStatus(int tsID)
{
  ...
  if (tsID == -1)
  {
    ...
    {
      std::fprintf(stdout, "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
      fflush(stdout);
    }
  ...
  else
  {
    if (!counthead)
    {
      if (cdo::stdoutIsTerminal) std::fprintf(stdout, " Process timestep :       ");
    ...
    if (cdo::stdoutIsTerminal)
    {
      std::fprintf(stdout, "\b\b\b\b\b\b%6d", tsID);
      fflush(stdout);
    }
  }
}
...
after_setNextDate(AfterControl *globs)
{
  ...
  while (true)
  {
    numFields = streamInqTimestep(globs->istreamID, TsID);
    if (numFields == 0 && (globs->Multi > 0 || globs->Nfiles > 0))
    {
...
after_readTimestep(ReadArgs &readArgs)
{
  ...
  while (numFields--)
  {
    streamInqField(globs->istreamID, &varID, &levelID);

    auto code = vlistInqVarCode(globs->ivlistID, varID);
    if (code <= 0 || code >= MaxCodes) continue;

    // Skip fields containing unneeded codes
    ...
    // Skip fields with unselected levels
    int levelOffset = -1;
    ...
    if (globs->Debug)
    {
      std::fprintf(stderr, "T%d", globs->Truncation);

      std::fprintf(stderr, "  Code %3d   Level%6d   %6.4d-%2.2d-%2.2d  %2.2d:%2.2d:00\n", code,
    ...
    if (analysisData)
    {
      streamReadField(globs->istreamID, globs->varray.data(), &numMissVals);
      after_AnalysisAddField(globs, vars, code, gridID, zaxisID, levelID, numMissVals);
    }
    else
    {
      double *dataptr = after_get_dataptr(vars, code, gridID, zaxisID, levelID);
      streamReadField(globs->istreamID, dataptr, &numMissVals);
      after_EchamAddField(globs, vars, code, gridID, zaxisID, levelID, numMissVals);
    }
...
static void
after_setEndOfInterval(AfterControl &globs, int numFields)
{
  if (numFields == 0)
  {
...
after_setLevel(AfterControl &globs)
{
  ...
  if (globs.NumLevelRequest == 0)
  {
    if (iVertID == -1)
    {
      if (globs.Verbose) std::fprintf(stdout, " No level detected\n");
    }
    else
    {
      if (Lhybrid2pressure)
      {
        if (globs.unitsel == 0)
        {
          if (globs.Verbose) std::fprintf(stdout, " Default pressure level selected:\n");
        ...
        {
          if (globs.Verbose) std::fprintf(stdout, " Default height level selected:\n");
      ...
      else
      {
        if (globs.Verbose)
        {
          if (iLevelType == ZAXIS_HYBRID)
            std::fprintf(stdout, " All detected hybrid level selected:\n");
          else
            std::fprintf(stdout, " All detected pressure level selected:\n");
        }
  ...
  else
  {
    ...
    {
      if (globs.Verbose) std::fprintf(stdout, " No level selected\n");
      globs.NumLevelRequest = 0;
      checkLevel = false;
    }
    else if (globs.Verbose)
    {
      if (Lhybrid2pressure) { std::fprintf(stdout, " Selected %s level:\n", (globs.unitsel == 0) ? "pressure" : "height"); }
      else
      {
        if (iLevelType == ZAXIS_HYBRID)
          std::fprintf(stdout, " Selected hybrid level:\n");
        else
          std::fprintf(stdout, " Selected %s level:\n", (globs.unitsel == 0) ? "pressure" : "height");
      }
    }
  }

  if (globs.Verbose && iVertID != -1)
    for (int l = 0; l < globs.NumLevelRequest; ++l) std::fprintf(stdout, "  Level %2d = %13.4f\n", l + 1, globs.LevelRequest[l]);
  ...
  if (checkLevel)
  {
    ...
    for (int k = 0; k < globs.NumLevelRequest; ++k)
    {
      ...
      for (int l = k + 1; l < globs.NumLevelRequest; ++l)
        ...
        {
          if (globs.Verbose) std::fprintf(stdout, "  Level %2d = %13.4f double request\n", l + 1, globs.LevelRequest[l]);
          removeLevel[l] = true;
        }
    }
    ...
    if (globs.AnalysisData || globs.Type < 30)
    {
      ...
      for (int k = 0; k < globs.NumLevelRequest; ++k)
      {
        ...
        if (!found)
        {
          std::fprintf(stdout, "  Level %2d = %14.4f not in input\n", k + 1, globs.LevelRequest[k]);
          removeLevel[k] = true;
        }
      }
...
after_setCodes(const AfterControl &globs, struct Variable *vars, int maxCodes, int numCodes)
{
  ...
  if (numCodes == 0)
  {
    if (globs.Verbose) std::fprintf(stdout, " All detected codes selected:\n");

    for (int code = 0; code < maxCodes; ++code)
      if (vars[code].detected) vars[code].selected = 1;
  }
  else if (globs.Verbose)
    std::fprintf(stdout, " Selected codes:\n");

  if (globs.Verbose)
  {
    std::fprintf(stdout, "  Table Code Name              Longname\n");
    std::fprintf(stdout, "  ----- ---- ----              --------\n");
  }
  ...
  for (int code = 0; code < maxCodes; ++code)
    if (vars[code].selected)
    {
      ...
      if (globs.Verbose)
      {
        std::fprintf(stdout, " %5d", table);
        std::fprintf(stdout, " %4d", code);
        if (!name[0])
          std::fprintf(stdout, "  var%d", code);
        else
        {
          std::fprintf(stdout, "  %-16s", name);
          if (longname[0]) std::fprintf(stdout, "  %s", longname);
        }
        std::fprintf(stdout, "\n");
      }
    }
...
after_parini(AfterControl &globs, struct Variable *vars)
{
  ...
  {
    std::fprintf(stderr, "Default namelist: \n");
  ...
  else
  {
    ...
    if (length == 0L) std::fprintf(stderr, "\n stdin not connected\n");
    std::fseek(stdin, 0L, SEEK_SET);
  }
  ...
  {
    lprintf(stderr);
    std::fprintf(stderr, "  Length of namelist:%4d bytes\n", (int) std::strlen(namelist));

    for (i = 0; i < (int) std::strlen(namelist); i += 60) std::fprintf(stderr, "  namelist[%02d]=%-60.60s\n", i, namelist + i);
    lprintf(stderr);
  }
...
after_dimcalc(AfterControl &globs)
{
  ...
  if (globs.AnalysisData) std::fprintf(stdout, " Found Ana or Re-Ana Data\n");

  if (globs.Verbose)
  {
    std::fprintf(stdout, " Dimensions:\n");
    std::fprintf(stdout, "  Truncation        = %4d\n", globs.Truncation);
    std::fprintf(stdout, "  Levels            = %4d\n", globs.NumLevel);
    std::fprintf(stdout, "  Latitudes         = %4d\n", globs.Latitudes);
    std::fprintf(stdout, "  Longitudes        = %4d\n", globs.Longitudes);
    lprintf(stdout);
  }
}
...
after_precntl(AfterControl &globs, struct Variable *vars)
{
  ...
  auto numVars = vlistNvars(globs.ivlistID);
  auto numGrids = vlistNumGrids(globs.ivlistID);
  auto numZaxes = vlistNumZaxis(globs.ivlistID);
  auto numSteps = vlistNtsteps(globs.ivlistID);

  if (globs.Debug)
  {
    std::fprintf(stderr, "numVars    = %d\n", numVars);
    std::fprintf(stderr, "numGrids   = %d\n", numGrids);
    std::fprintf(stderr, "numZaxes   = %d\n", numZaxes);
    std::fprintf(stderr, "numSteps   = %d\n", numSteps);
  }
  ...
  if (globs.Truncation == 0)
  {
    if (globs.Latitudes)
    {
      switch (globs.Latitudes)
      {
        ...
        case 1: break;
        default: std::fprintf(stderr, "%d Gaussian latitudes not supported.\n", globs.Latitudes);
      }
    }
  }
  ...
  if (globs.Debug) std::fprintf(stderr, "   T = %3d   L = %2d\n", globs.Truncation, globs.NumLevelFound);

  if (globs.Debug) std::fprintf(stderr, " CODE CHECK\n");
  ...
  if (globs.Verbose)
  {
    ...
    else
    {
      if (institutInqLongnamePtr(instID))
        ...
        std::fprintf(stdout, "name unknown\n");
    }

    std::fprintf(stdout, " Source    : ");
  ...
  for (int varID = 0; varID < numVars; ++varID)
  {
    ...
    if (globs.Debug)
      std::fprintf(stderr, "Code %3d  Levels = %3d  LevelType = %3d  GridType = %3d\n", code, numlevel, leveltype, gridtype);
  }

  if (globs.Debug) std::fprintf(stderr, "FieldDim = %d\n", gridSizeMax);
  ...
  if (globs.Debug)
    for (int code = 0; code < MaxCodes; ++code)
    {
      if (vars[code].detected) std::fprintf(stderr, " Detected Code %3d with %3d level\n", code, zaxisInqSize(vars[code].izaxisID));
    }
}
...
after_postcntl(const AfterControl &globs, struct Variable *vars)
{
  ...
  if (globs.Debug)
    for (int code = 0; code < MaxCodes; ++code)
      if (vars[code].detected)
      {
        ...
        std::fprintf(stderr, " Detected Code %3d  grid %-8s size %5zu  level %2d %-8s\n", code, gridNamePtr(gridInqType(gridID)),
  ...
  if (globs.Debug)
    for (int code = 0; code < MaxCodes; ++code)
      if (vars[code].needed) { std::fprintf(stderr, "   Needed Code %3d\n", code); }
  ...
  if (globs.Debug)
    for (int code = 0; code < MaxCodes; ++code)
      if (vars[code].selected)
      {
        ...
        zaxisName(zaxisInqType(zaxisID), zaxistypename);
        std::fprintf(stderr, " Selected Code %3d  grid %-8s size %5zu  level %2d %-8s\n", code, gridNamePtr(gridInqType(gridID)),
                     gridInqSize(gridID), zaxisInqSize(zaxisID), zaxistypename);
      }
}

static void
after_readVct(AfterControl &globs, std::string const &vctfile)
{
  char line[1024];
  int nlines = 0;

  auto fobj = c_fopen(vctfile, "r");
  if (fobj.get() == nullptr) cdo_sys_error("Open failed on %s", vctfile);

  while (fgets(line, 1023, fobj.get()))
  {
  ...
  std::fprintf(stdout, "  Read VCT with %d hybrid levels from file %s\n", globs.numHalfLevels - 1, vctfile.c_str());
}
...
public:
  ...
  init() override
  {
    ...
    if (cdo_operator_argc() == 1) after_readVct(globs, cdo_operator_argv(0));
