...
{
  (varName) ? fprintf(stdout, "%s@%s = ", varName, attname) : fprintf(stdout, "%s = ", attname);
}
...
print_attr_special(const char *varName, const CdoVars &cdoVars, int vlistID, int varOrGlobal, const char *argument)
{
  ...
  if (argument)
  {
    ...
    if (var.units.size() && wildcard_match("units", argument)) fprintf(stdout, "%s@units = \"%s\"\n", varName, var.units.c_str());
    if (wildcard_match("missing_value", argument)) fprintf(stdout, "%s@missing_value = %g\n", varName, var.missval);
    if (haveAddoffset && wildcard_match("add_offset", argument)) fprintf(stdout, "%s@add_offset = %g\n", varName, addoffset);
