...
const CdoHelp ShowinfoHelp = {
    ...
    "    showmon, showdate, showtime, showtimestamp, showchunkspec, showfilter - ",
    ...
    "    Depending on the chosen operator the name, level, date, time and other information is printed.",
    "",
    "OPERATORS",
...
const CdoHelp SetchunkspecHelp = {
    ...
    "",
    "DESCRIPTION",
    "    Specify chunking for selected variables in the output. Chunking is available for NetCDF4 and useful to specify the units of disk access and compression.",
    "    The filename parameter is used to specify the file which contains the chunk specification for each variable.",
    "    The chunkspec argument is a comma-separated string with the chunk size for the dimensions x,y,z,t.",
    "    A chunkspec must name at least one dimension, e.g. t=<chunksize> to set the chunk size of the time dimension to <chunksize>.",
    "    ",
    "    Use the CDO option --chunkspec instead of setchunkspec if all variables require the same chunks.",
    "",
    "PARAMETER",
    "    filename  STRING  Name of the file containing the chunk specification per variable [format: varname=\"<chunkspec>\"]",
};
...
const CdoHelp SetfilterHelp = {
    ...
    "",
    "DESCRIPTION",
    "    Specify filter for selected variables in the output. Filters are available for NetCDF4 and mainly used to compress/decompress data.",
    ...
    "    ",
    "    The filename parameter is used to specify the file which contains the filter specification for each variable.",
    ...
    "",
    "PARAMETER",
    "    filename  STRING  Name of the file containing the filter specification per variable [format: varname=\"<filterspec>\"]",
};
...
const CdoHelp CollgridHelp = {
    ...
    "    structured lat/long grid box. Data on an unstructured grid are concatenated in the order of the input files.",
    "    For ICON restart data, the array global_cell_indices is used for indexing if it is available.",
    ...
    "    gridtype  STRING   For unstructured grids, set to unstructured.",
    "",
    "NOTE",
...
const CdoHelp SetHelp = {
    "NAME",
    "    setcodetab, setcode, setparam, setname, setstdname, setunit, setlevel, ",
    "    setltype, setmaxsteps - Set field info",
    "",
    "SYNOPSIS",
    ...
    "                 Sets the standard name of the first variable.",
...
const CdoHelp FldstatHelp = {
    ...
    "",
    "PARAMETER",
    "    verbose  BOOL   print lon/lat coordinates of min/max values",
    "    weights  BOOL   weights=FALSE disables weighting by grid cell area [default: weights=TRUE]",
    "    pn       FLOAT  Percentile number in {0, ..., 100}",
};
...
const CdoHelp VertintmlHelp = {
    ...
    "    A basic linear method is used for interpolation.",
    ...
    "    ",
    "    Use the alias ml2plx/ml2hlx to fill in missing values with the next available value of the same",
    "    vertical column. Only the temperature is extrapolated in this case. The extrapolation method originates from",
    "    the ECHAM postprocessing.",
    "    This operator requires all variables on the same horizontal grid. Missing values in the input data are not supported.",
    "    ",
    "",
    "OPERATORS",
    ...
    "",
    "NOTE",
    "    This is a specific implementation for data from the ECHAM model, it may not work with data from other sources.",
    "    The components of the hybrid coordinate must always be avaiable at the hybrid layer interfaces even if the",
    "    data is defined at the hybrid layer midpoints.",
};
...
const CdoHelp VertintapHelp = {
    ...
    "    A basic linear method is used for interpolation. The input file must contain the",
    "    3D air pressure in pascal. The air pressure is identified by the NetCDF CF standard name air_pressure.",
    "    Use the alias ap2plx to fill in missing values with the next available value of the same",
    "    vertical column. This operator requires all variables on the same horizontal grid.",
    "    Missing values in the input data are not supported.",
    "",
    "PARAMETER",
    "    levels  FLOAT  Comma-separated list of pressure levels in pascal",
    "",
    "NOTE",
...
const CdoHelp VertintghHelp = {
    ...
    "    A basic linear method is used for interpolation. The input file must contain the",
    "    3D geometric height in meter. The geometric height is identified by the NetCDF CF",
    "    standard name geometric_height_at_full_level_center.",
    "    Use the alias gh2hlx to fill in missing values with the next available value of the same",
    "    vertical column. This operator requires all variables on the same horizontal grid.",
    "    Missing values in the input data are not supported.",
    "",
    "PARAMETER",
    "    levels  FLOAT  Comma-separated list of height levels in meter",
    "",
    "NOTE",
    "    This is a specific implementation for NetCDF files from the ICON model, it may not work with data from other sources.",
};
...
const CdoHelp DeriveparHelp = {
    ...
    "",
    "DESCRIPTION",
    "    This module contains operators that calculate derived model parameters. All necessary input variables are",
    "    identified by their GRIB1 code number or the NetCDF CF standard name.",
    ...
    "                      Required input fields are surface_air_pressure, specific_humidity and air_temperature on full",
    "                      hybrid sigma pressure levels. The air density (rho) is calculated with the following formula:",
    "                      ",
    "                              rho = P  / Rs * Tv",
    "                      ",
