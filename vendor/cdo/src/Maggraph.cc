         ...
         std::vector<std::string> &params)
{
  ...
  {
    ntime_steps = nts[0];
    fprintf(stderr, " numFiles=%ld  numSteps=%ld\n", numFiles, ntime_steps);
  ...
  if (stat)
  {
    ...
    for (tsID = 0; tsID < ntime_steps; ++tsID)
    {
      ...
      {
        if (DBG) fprintf(stderr, "fileIdx=%ld  ", fileIdx);
        ...
        if (DBG)
        {
          fprintf(stderr, " %6g", datatab[fileIdx][tsID]);
          fprintf(stderr, "\n");
        }
      }
    }
    ...
    */
    ...
    for (fileIdx = 0; fileIdx < numFiles; ++fileIdx)
    {
      ...
      for (tsID = 0; tsID < nts[fileIdx]; ++tsID)
      {
        ...
        if (DBG && (tsID == 0 || tsID == nts[fileIdx] - 1)) fprintf(stderr, "%s\n", date_time_str[fileIdx][tsID]);
