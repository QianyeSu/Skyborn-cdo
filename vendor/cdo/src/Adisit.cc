...
IOSettings
configureOutput(const std::function<void(int, int)> &outputSettingFunc, VarList const &varList, int vlistID, int thoID, int saoID,
...
public:
  ...
  init() override
  {
    ...
    auto outputSetting_ADISIT = [](int p_vlistID2, int p_tisID2)
    {
    ...
    auto outputSetting_ADIPOT = [](int p_vlistID2, int p_tisID2)
    {
    ...
    numLevels = configResults.nlevels;
      ...
      while (numFields--)
      {
        ...
        if (varID == thoID)
        {
          ...
          if (mm.min < MIN_T || mm.max > MAX_T)
            cdo_warning("Temperature in degree Celsius out of range (min=%g max=%g) [timestep:%d levelIndex:%d]!", mm.min, mm.max,
                        tsID + 1, levelID + 1);
        }
      }

      if (operatorID == ADISIT)
        calc_adisit(gridsize, numLevels, pressure, tho, sao, tis);
      else
        calc_adipot(gridsize, numLevels, pressure, tho, sao, tis);
